...
import { getNextSpacingDateForRem, updateSRSDataForRem, calculateReviewDaysFromPriority } from './scheduler';
...
export async function initIncrementalRem(plugin: RNPlugin, rem: Rem) {
  await rem.addPowerup(powerupCode);
  
  // 只添加 powerup，不设置任何初始值
